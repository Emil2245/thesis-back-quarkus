...
> Plan 022 DONE**; DAG I-07 sigue desbloqueado. Plan 022 implementación
> aplicada (P-28 completo): `CapituloResource` dedicado con
> `presupuestoId` en cada ruta, numeración jerárquica autogenerada,
> renumeración atómica en dos pasadas con entidades gestionadas
> (item temporal `~<id>` para no violar `UNIQUE (presupuesto_id,
> item)` durante el flush), prevención de ciclos self/descendiente
> con `CapituloRepository.esDescendienteOigual` (STOP (A) activado —
...
> que devuelve el árbol completo (`PresupuestoResponse`). Planes
> 023–025 siguen **PLANNED / READY** (sin bloqueo transitivo);
> **siguiente plan ejecutable = Plan 023** (`05-rubros-totales-resumen.md`).
