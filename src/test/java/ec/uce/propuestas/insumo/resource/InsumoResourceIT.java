...
class InsumoResourceIT {
    ...
    void reset() throws Exception {
        ...
        try (Connection con = ds.getConnection(); Statement st = con.createStatement()) {
            st.execute("TRUNCATE TABLE insumo, base_insumos, parametros_proyecto, firmante, proyecto, " +
    ...
    void TC_P13_crear_y_listar_insumos_de_la_base_del_proyecto() {
        ...
        given().contentType(JSON)
                .header("Authorization", "Bearer " + token)
                .body(Map.of("codigo", "MAT-001", "tipo", "MATERIAL", "descripcion", "Cemento",
                        "unidad", "kg", "precioUnitario", 0.650000))
                .when().post("/api/v1/proyectos/" + proyecto + "/insumos")
                .then().statusCode(201)
                .body("codigo", equalTo("MAT-001"))
                .body("tipo", equalTo("MATERIAL"));

        given().header("Authorization", "Bearer " + token)
                .when().get("/api/v1/proyectos/" + proyecto + "/insumos")
                .then().statusCode(200)
    ...
    void TC_P13_codigo_duplicado_en_una_base_devuelve_400() {
        ...
        given().contentType(JSON).header("Authorization", "Bearer " + token)
                .body(Map.of("codigo", "DUP-1", "tipo", "MATERIAL", "descripcion", "A",
                        "unidad", "kg", "precioUnitario", 1.0))
                .when().post(url).then().statusCode(201);
    ...
    void TC_P06_fuerza_unidad_h_para_mano_de_obra() {
        ...
        given().contentType(JSON).header("Authorization", "Bearer " + token)
                .body(Map.of("codigo", "MO-1", "tipo", "MANO_OBRA", "descripcion", "Peón",
                        "unidad", "h", "precioUnitario", 3.50))
                .when().post("/api/v1/proyectos/" + proyecto + "/insumos")
                .then().statusCode(201);
    }
}