...
class SchemaBaselineIT {
    ...
    private static final Set<String> TABLES =
            names("usuario,refresh_token,token_usuario,proyecto,firmante,parametros_sistema,parametros_proyecto,"
    ...
    private static final Set<String> PUBLIC_TABLES = names(
            "usuario,firmante,proyecto,presupuesto,apu,apu_detalle,base_insumos,insumo,plantilla_apu,plantilla_proyecto");
    private static final Set<String> INTERNAL_TABLES =
            names("refresh_token,token_usuario,parametros_sistema,parametros_proyecto,apu_seccion,"
    ...
    private static final Set<String> IDENTITY_TABLES = names(
            "usuario,refresh_token,token_usuario,proyecto,firmante,base_insumos,insumo,presupuesto,"
    ...
    private static final Set<String> RANGES = names(
            "rango_hm_min,rango_hm_max,rango_ci_min,rango_ci_max,rango_descuento_min,rango_descuento_max,rango_iva_min,rango_iva_max");
    ...
    void relational_identity_and_foreign_keys_remain_bigint() throws Exception {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            while (rows.next())
                assertEquals("bigint", rows.getString("data_type"), rows.getString(1) + "." + rows.getString(2));
        }
    }
    ...
    void ranges_et_templates_discount_and_inert_seams_are_structural() throws Exception {
        ...
        assertColumns(
                "proyecto:titulo_et_1:text,proyecto:titulo_et_2:text,proyecto:plantilla_proyecto_origen_id:bigint,"
    ...
    void public_id_immutability_trigger_is_installed_on_each_public_table() throws Exception {
        ...
        assertTrue(
                exists(
                        "SELECT 1 FROM pg_proc WHERE proname='fn_assert_public_id_immutable' AND prorettype='trigger'::regtype"));
    }
    ...
    private Set<String> tablesWithColumn(String name) throws Exception {
        return querySet(
                "SELECT table_name FROM information_schema.columns WHERE table_schema=current_schema() AND column_name=?",
                name);
    }

    private Set<String> columnNames(String table, String pattern) throws Exception {
        return querySet(
                "SELECT column_name FROM information_schema.columns WHERE table_schema=current_schema() AND table_name=? AND column_name LIKE ?",
                table,
                pattern);
    }
    ...
    private Column column(String table, String name) throws Exception {
        String sql =
                "SELECT data_type,is_nullable,column_default,is_identity,numeric_precision,numeric_scale FROM information_schema.columns "
    ...
    private boolean hasUniqueConstraint(String table, String column) throws Exception {
        return exists(
                "SELECT 1 FROM information_schema.table_constraints tc JOIN information_schema.key_column_usage kcu "
                        + "ON tc.constraint_name=kcu.constraint_name AND tc.table_schema=kcu.table_schema WHERE tc.table_schema=current_schema() "
                        + "AND tc.table_name='" + table + "' AND tc.constraint_type='UNIQUE' AND kcu.column_name='"
                        + column + "'");
    }
        ...
        for (String specification : specifications.split(",")) {
            ...
            String sql =
                    "SELECT 1 FROM information_schema.table_constraints tc JOIN information_schema.key_column_usage kcu "
                            ...
                            + "AND tc.table_name='" + fields[0]
                            + "' AND tc.constraint_type='FOREIGN KEY' AND kcu.column_name='" + fields[1]
    ...
    private record Column(
            String type, String nullability, String defaultExpression, String identity, int precision, int scale) {}
