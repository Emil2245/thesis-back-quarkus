 ...
 * autor. No expone IDs internos BIGINT ni campos sensibles. Plan 044 —
 * {@code tipo} distingue SISTEMA (solo lectura para USUARIO) de PERSONAL.
 */
...
public record PlantillaProyectoResponse(
        ...
        String descripcion,
        JsonNode snapshotEstructura,
        Instant fechaCreacion) {

    public static PlantillaProyectoResponse from(PlantillaProyecto p, JsonNode snapshot) {
        return new PlantillaProyectoResponse(p.publicId, p.nombre, p.tipo, p.descripcion, snapshot, p.fechaCreacion);
    }
}
