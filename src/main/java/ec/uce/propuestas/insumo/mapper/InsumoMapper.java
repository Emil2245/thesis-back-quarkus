...
public final class InsumoMapper {
    ...
    public static InsumoResponse toResponse(Insumo e) {
        boolean desactualizado = e.updatedAt == null
                || e.updatedAt.isBefore(Instant.now().minusSeconds(DESACTUALIZADO_DAYS * 86400L));
        return new InsumoResponse(
                e.id, e.codigo, e.tipo, e.descripcion, e.unidad,
                e.precioUnitario, e.updatedAt, desactualizado);
    }
}